...
var Model = exports.Model = function(schema, name, fields, options){
     ...
     * @type {Array.<{ load: Function, save: Function, remove: Function }>?}
     * @private
     */
...
Model.prototype.findOne = function(criteria, fields, sort, options){
    ...
    return Q()
        ...
        // loadRelated
        .then(self._withRelated('load')) // -> entity
        // afterFindOne
        .then(function(entity){
...
Model.prototype.find = function(criteria, fields, sort, options){
    ...
    return Q()
        ...
        // loadRelated
        .then(self._withRelated('load')) // -> entities
        // afterFind
        .then(function(entities){
            ctx.entities = entities;
...
Model.prototype.insert = function(entities, options){
    ...
    return Q()
        ...
        // saveRelated
        .then(self._withRelated('save')) // -> entities
        // afterInsert
        .then(function(entities){
            ctx.entities = entities;
...
Model.prototype.remove = function(entities, options){
    ...
    return Q()
        ...
        // removeRelated
        .then(self._withRelated('remove')) // -> entities
        // remove
        .then(function(){
...
Model.prototype.withRelated = function(prop){
    ...
    model._queryWith.push( _.toArray(arguments) );
    return model;
};

/** Process relation handling methods assigned by Model.withRelated()
 * @param {String} name
 *      Relation processing method to invoke: loadRelated, saveRelated, removeRelated
 * @returns {function(Object|Array.<Object>):Q} -> entity | entities
 * @protected
 */
Model.prototype._withRelated = function(name){
    var self = this;
    return function(entities){
        if (!self._queryWith || _.isEmpty(entities))
            return entities;

        return _.map(self._queryWith, function(args){
            return function(){
                return self.doRelated.apply(self, [name, entities].concat(args));
            };
        }).reduce(Q.when, Q(1))
