 ...
 *      Model options
 * @property {Object.<IMissyRelation>} relations
 *      Model relations
    ...
    return Q()
        ...
        .then(function(entity){
            if (!self._queryWith || !entity)
                return entity;
            return _.map(self._queryWith, function(f){ return _.partial(f, entity); })
                .reduce(Q.when, Q(1))
    ...
    return Q()
        ...
        .then(function(entities){
            if (!self._queryWith || !entities.length)
                return entities;
            return _.map(self._queryWith, function(f){ return _.partial(f, entities); })
                .reduce(Q.when, Q(1))
...
Model.prototype._getDeepRelation = function(entities, prop){
    ...
    _.each(propsPath, function(prop, level){
        if (!(prop in model.relations))
            throw new errors.MissyModelError(model, 'Undefined deep relation: ' + prop);

        // Follow the relation chain
        relation = model.relations[prop];
        ...
        if (relation.arrayRelation)
            _.each(rows, function(row){
                if (_.isArray(row[prop]))
                    newRows = newRows.concat(row[prop]);
            });
        else
            _.each(rows, function(row){
                if (!_.isEmpty(row[prop]))
                    newRows.push(row[prop]);
            });
 ...
 *
 * @see {IMissyRelation#loadRelated}
 * @see {Model#find}
 *
 * @param {Object|Array.<Object>} entities
 ...
 *      Supports deep eager loading using the '.'-notation: User.withRelated('posts.images').find(...)
 * @param {String|Object|MissyProjection?} fields
 *      Related search fields projection
 * @param {String|Object|Array|MissySort?} sort
 *      Related search sorting
 * @param {Object?} options
...
Model.prototype.loadRelated = function(entities, prop, fields, sort, options){
    ...
    var relInfo = this._getDeepRelation([].concat(entities), prop);

    return relInfo.relation.loadRelated.apply( relInfo.relation,
        [relInfo.rows].concat(_.toArray(arguments).slice(2))
    ).then(function(){
        return entities;
    });
};
Model.prototype.loadRelated = Model._whenConnected(Model.prototype.loadRelated);

/** Eagerly load related entities with the sebsequent query.
 * Can be called multiple times.
 *
 * Only works with: find(), findOne(). Will you ever need more? :)
 *
 ...
 *      Supports deep eager loading using the '.'-notation.
 * @param {String|Object|MissyProjection?} fields
 * @param {String|Object|Array|MissySort?} sort
 * @param {Object?} options
 *
