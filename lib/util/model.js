...
var MissyCriteria = exports.MissyCriteria = function(model, criteria){
    ...
    this.criteria = _.transform(criteria || {}, function(res, value, fieldName){
        // Normalize
        if (!_.isObject(value) || !_.any(value, function(v, operator){ return operator[0]==='$'; }))
            value = { $eq: value };

        // Convert
        var convert = _.partial(model.converter.convertValue.bind(model.converter), fieldName, 'save');

        value = _.transform(value, function(res, operand, operator){ // walk through operands and convert them
            // Known operator?
...
MissyCriteria.fromPk = function(model, pk){
    ...
    if (_.isUndefined(pk) || _.isNull(pk))
        throw new errors.MissyModelError(this.model, 'Empty primary key given');
    ...
    if (_.isArray(pk)){
        if (pk.length !== this.options.pk.length)
            throw new errors.MissyModelError(this.model, 'Inconsistent primary key fields count');
        return _.object(this.options.pk, pk); // ordinal zip
    }

    // object PK
    return new MissyCriteria(pk);
};
