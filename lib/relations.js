    ...
    return Q()
        ...
        .then(function(relatedEntities){
            ...
            _.each(relatedEntities, function(related){
                // Locate an entity
                id = identity(related, foreignFields);
    ...
    return Q()
        ...
        .then(function(){
            if (hostFields.length === 1){
                ...
                var criteria = {};
                criteria[ foreignFields[0] ] = { $in: _.pluck(entities, foreignFields[0]) };
                return self.foreign.removeQuery(criteria);
            } else {
                ...
                return self.model.loadRelated( _.cloneDeep(entities), foreignFields )
        ...
        // Save the new set of related entities
        .then(function(){
            var relatedEntities = [];
...
hasOne.prototype.removeRelated = function(entities, options){
    ...
    // Collect the related entities
    var relatedEntities = _.flatten(_.compact(_.pluck(entities, self.prop)), true);

    // Remove them
    return self.foreign.remove(entities, options)
        .thenResolve(entities);
};
