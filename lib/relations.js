...
var hasOne = exports.hasOne = function(model, prop, foreign, fields){
    ...
    if (!(foreign instanceof missy.Model))
        throw new errors.MissyRelationError(this, 'Foreign object is not a model');

    (function(unknownFields){
        if (unknownFields.length)
            throw new errors.MissyRelationError(self, 'Unknown source fields in relation: ' + unknownFields.join(', '));
    })(_.difference(_.keys(this.fields), _.keys(model.fields)));

    (function(unknownFields){
        if (unknownFields.length)
            throw new errors.MissyRelationError(self, 'Unknown target fields in relation: ' + unknownFields.join(', '));
    })(_.difference(_.values(this.fields), _.keys(foreign.fields)));
};
